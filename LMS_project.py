...
class Book:
    ...
    def get_inp(self):
        ...
        self.pubyear = input("ENTER THE PUBLICATION YEAR: ")
        while True:
            try:
...
class Library:
    def __init__(self):
        self.books = []
        try:
            with open("libb_data.json", 'r') as file:
                self.books = json.load(file)
                print("File loaded successfully")
        except FileNotFoundError:
            print("libb_data.json file is not found.")
    ...
    def search_book(self):
        ...
        if results:
            for idx, book in enumerate(results, start=1):
                ...
                print(f"  Published Year: {book.get('pubyear', 'N/A')}")
        ...
        for book in self.books:
            if book['isbn'] == isbn:
                if book['borrowed_by']:
                    ...
                    book['borrowed_date'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    def save_data(self):
        with open("libb_data.json", 'w') as file:
